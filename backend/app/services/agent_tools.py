...
async def file_complaint(
    ...
    # Mirror the complaint into the municipality dashboard's issues store.
    # The dashboard reads from a Next.js in-memory store at /api/water/issues,
    # completely separate from the SQLite WaterComplaint table. Without this
    # call, agent-filed complaints never appear there.
    #
    # FRONTEND_URL is read from settings so it tracks whichever port Next.js
    # is actually running on (set FRONTEND_URL=http://localhost:3010 in .env
    # if Next.js started on a non-default port).
    from app.config import settings as _settings
    _ISSUES_URL = f"{_settings.frontend_url.rstrip('/')}/api/water/issues"
    # Map VALID_COMPLAINT_TYPES to the WaterIssue type enum
    _TYPE_MAP = {
        "no-supply":     "supply_disruption",
        "low-pressure":  "supply_disruption",
        "leakage":       "leakage",
    ...
    try:
        ...
        async with _httpx.AsyncClient(timeout=4) as _client:
            await _client.post(
                _ISSUES_URL,
                json={
                    "ward_name":   ward.name,
                    "type":        _TYPE_MAP.get(cat, "other"),
                    "description": desc,
                    "reported_by": "agent",   # visually honest source tag
                    "status":      "Open",
                },
            )
    except Exception as _e:
        # Non-fatal — the complaint is already in the DB; the mirror is best-effort.
        # Log but don't fail the tool response.
        import logging as _log
        _log.getLogger(__name__).warning(
            "file_complaint: could not mirror to issues store at %s: %s", _ISSUES_URL, _e
        )
...
async def get_water_schedule(db: Session, ward_name: str) -> dict[str, Any]:
    """
    Return today's water supply schedule for a named ward.
    The agent can answer "when is my water coming?" using this.
    """
    ward = resolve_ward(db, ward_name)
    if not ward:
        return _ward_not_found(ward_name, db)
    schedule = await water.get_today_schedule(db)
    ward_entry = next((s for s in schedule if s.get("ward_id") == ward.id), None)
    if not ward_entry:
        return {
            "ok": False,
            "error": "no_schedule",
            "ward_name": ward.name,
            "message": f"No supply schedule found for {ward.name} today.",
        }
    return {
        "ok": True,
        "ward_id": ward.id,
        "ward_name": ward.name,
        "supply_today": ward_entry.get("supply_today"),
        "supply_start_time": ward_entry.get("supply_start_time"),
        "supply_end_time": ward_entry.get("supply_end_time"),
        "days_since_supply": ward_entry.get("days_since_supply"),
        "fairness_score": ward_entry.get("fairness_score"),
        "reasoning": ward_entry.get("reasoning"),
    }


async def triage_ward_complaint(
    db: Session, ward_name: str, category: str, description: str
) -> dict[str, Any]:
    """
    Run AI triage on a specific complaint — returns severity, recommended team,
    ETA, and Gemma reasoning grounded in ward history.
    """
    ward = resolve_ward(db, ward_name)
    if not ward:
        return _ward_not_found(ward_name, db)
    ...
    if not desc:
        return {"ok": False, "error": "empty_description", "message": "description is required."}
    result = await water.triage_complaint_with_precedent(
        db=db, description=desc, issue_type=cat, ward_id=ward.id
    )
    return {
        "ok": True,
        "ward_id": ward.id,
        "ward_name": ward.name,
        **{k: v for k, v in result.items() if k != "retrieved_cases"},
        "precedent_cases_used": len(result.get("retrieved_cases") or []),
    }


async def get_budget_project_status(db: Session, ward_name: str) -> dict[str, Any]:
    """
    Return budget project status for a ward — with AI anomaly detection and
    promise-vs-reality summaries from the BudgetGemmaService.
    """
    ...
    projects = db.query(BudgetProject).filter(BudgetProject.ward_id == ward.id).all()
    if not projects:
        return {
            "ok": True,
            "ward_id": ward.id,
            "ward_name": ward.name,
            "message": f"No budget projects found for {ward.name}.",
            "projects": [],
        }

    svc = BudgetGemmaService()
    summaries = []
    for p in projects[:5]:  # cap at 5 to stay within context
        # Use cached gemma_summary if available
        summary_text = p.gemma_summary or svc._fallback_summary(p)
        anomaly_flag = p.gemma_anomaly_flag or "none"
        anomaly_explanation = p.gemma_anomaly_explanation or ""
        if not anomaly_flag or anomaly_flag == "none":
            anomaly_flag, anomaly_explanation = svc._fallback_anomaly_detection(p)
        summaries.append({
            "project_id": p.id,
            "name": p.project_name,
            "category": p.category,
