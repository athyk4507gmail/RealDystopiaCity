...
You MUST always return a JSON object with a specific, reasoned recommendation — even with no historical cases.

If historical cases ARE provided:
- Base severity and ETA on them. Weight same-ward precedents over citywide ones.
- In your reasoning, cite specific case numbers ("Case 1", "Case 2") including resolution time and team if available.
- Do not invent past cases that weren't given to you.

If NO historical cases are provided (or fewer than 2):
- Do NOT use generic boilerplate. Reason specifically from the complaint description, type, and ward stats given.
- Infer severity from keywords in the description (e.g. "burst pipe" → high, "discoloration" → medium, "low pressure" → low-medium).
- Infer a reasonable team assignment from the complaint type (leakage → Pipeline Team, contamination → Quality Team, no-supply → Distribution Team, low-pressure → Pressure Team).
- State explicitly in reasoning that there is no ward history, and explain your estimate from the complaint details.
...
Return ONLY valid JSON — no markdown fences, no prose outside the object:
{
  ...
  "reasoning": "2-3 sentences specific to this complaint",
...
async def triage_complaint_with_precedent(
    ...
    # Always build the prompt — Gemma handles zero-history via its system prompt instructions.
    if n_cases == 0:
        history_block = "No resolved cases on record for this ward and complaint type."
    else:
        history_block = f"Historical resolved cases ({n_cases}):\n{case_block}"
    ...
    try:
        ...
        logger.info("Gemma triage FULL raw response:\n%s", raw_response or "<empty>")
        result = gemma.parse_json(raw_response)
        if "severity" not in result:
            logger.warning(
                "Gemma triage response missing 'severity' key. raw=%r parsed=%r",
                raw_response[:500] if raw_response else None,
                result,
            )
            raise ValueError(f"missing severity; parsed keys: {list(result.keys())}")
        result["based_on_cases"] = int(result.get("based_on_cases", n_cases))
        # Bug B fix: ensure ETA range is not inverted (Gemma occasionally swaps low/high)
        eta_low = result.get("eta_hours_low", 6)
        eta_high = result.get("eta_hours_high", 12)
        if isinstance(eta_low, (int, float)) and isinstance(eta_high, (int, float)) and eta_low > eta_high:
            logger.warning("Gemma returned inverted ETA %s–%sh — swapping.", eta_low, eta_high)
            result["eta_hours_low"], result["eta_hours_high"] = eta_high, eta_low
        result["retrieved_cases"] = retrieved_cases
        result["fallback"] = False
        return result
    except Exception as exc:
        logger.warning(
            "Gemma triage failed — exc=%s raw_response=%r",
            exc,
            (raw_response or "")[:600],
            exc_info=True,
        )
        fallback = _fallback_triage(normalized_type, n_cases)
