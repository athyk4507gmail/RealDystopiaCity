...
export default function ComplaintsPage() {
  ...
  // Refetch when the user switches back to this tab — catches complaints filed via
  // the Civic Agent while the user was on the /agent page.
  useEffect(() => {
    const onVisible = () => {
  ...
  return (
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {/* Manual refresh — ensures complaints filed via the Civic Agent appear
            immediately without waiting for a full page reload. The page also
            auto-refreshes on tab focus via the visibilitychange listener above. */}
        <button
          onClick={load}
          disabled={loading}
          className="flex items-center gap-2 rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-slate-700 transition-colors disabled:opacity-50 shrink-0"
        >
